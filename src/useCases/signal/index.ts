...
function alertOnMattermost(payload: alertPayloadType) {
    ...
    const text = `Bonjour @anna-livia @julien.dauphant @ishan @florian.delezenne @benoit.serrano.\nUne alerte a été levée sur le repository ${payload.occurrence.source.url}. L'auteur du commit est ${payload.occurrence.author_info} : contactez cette personne pour plus d'informations sur la fuite.`;
